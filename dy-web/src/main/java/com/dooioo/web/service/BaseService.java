...
public abstract class BaseService<T> {
     ...
     * @return
     */
    public int count(String sqlId, Map<String, Object> bindParams) {
        sqlId = wrapSqlId(sqlId);
        return queryDao.count(sqlId, bindParams);
    }
     ...
     * @return
     */
    public int count(Map<String, Object> bindParams) {
        return count(Constants.DAO_COUNT, bindParams);
    }
    ...
    public boolean contains(BaseEntity entityObject) {
        return queryTopOne(entityObject) != null;
    }
    ...
    /**
     * 包裹ibatis的sqlId
     * query(sqlId("queryByName"), 1) => query("queryByName, 1)
     *
     * @param sqlId
     * @return
     */
    private final String wrapSqlId(String sqlId) {
        if(Strings.isEmpty(sqlId)) {
            return "";
        }

        if(Strings.contains(sqlId, ".")) {
            return sqlId;
        }

        return this.sqlId(sqlId);
    }
}
